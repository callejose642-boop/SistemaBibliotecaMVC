...
<head>
    <meta charset="UTF-8">
    <title>Sistema de Biblioteca - Reservas</title>
    ...
    <div class="row">
        <div class="col-md-3 col-lg-2 sidebar d-flex flex-column p-3 text-white bg-dark" style="min-height: 100vh;">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                ...
                <li><a href="index.php?url=reservas/listar" class="nav-link active"><i class="bi bi-bookmark-star"></i> Reservas</a></li>
        ...
        <div class="col-md-9 ms-sm-auto col-lg-10 main-content">
            <div class="d-flex justify-content-between pt-3 pb-2 mb-3 border-bottom">
                ...
                <a href="index.php?url=reservas/crearForm" class="btn btn-success"><i class="bi bi-plus-circle"></i> Nueva Reserva</a>
            </div>
            <div class="card shadow-sm"><div class="card-body"><table class="table table-hover table-bordered align-middle">
                <thead class="table-dark"><tr><th>ID</th><th>Libro</th><th>Lector</th><th>F. Reserva</th><th>Estado</th><th class="text-center">Acciones</th></tr></thead>
                <tbody>
                    <?php if(empty($reservas)): ?><tr><td colspan="6" class="text-center">No hay registros</td></tr><?php else: ?>
                    <?php foreach($reservas as $r): ?><tr><td><?= htmlspecialchars($r["id"]) ?></td><td><strong><?= htmlspecialchars($r["libro"]) ?></strong></td><td><?= htmlspecialchars($r["lector"]) ?></td><td><?= htmlspecialchars($r["fecha_reserva"]) ?></td>
                    <td><span class="badge <?= $r["estado"] == 'Completada' ? 'bg-success' : ($r["estado"] == 'Cancelada' ? 'bg-danger' : 'bg-primary') ?>"><?= htmlspecialchars($r["estado"]) ?></span></td>
                    <td class="text-center"><a href="index.php?url=reservas/editarForm&id=<?= $r["id"] ?>" class="btn btn-sm btn-primary"><i class="bi bi-pencil"></i></a> <a href="index.php?url=reservas/eliminarForm&id=<?= $r["id"] ?>" class="btn btn-sm btn-danger"><i class="bi bi-trash"></i></a></td></tr><?php endforeach; endif; ?>
                </tbody>
            </table></div></div>
        </div>
    </div>
