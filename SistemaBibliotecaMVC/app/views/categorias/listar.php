...
<head>
    <meta charset="UTF-8">
    <title>Sistema de Biblioteca - Categorías</title>
    ...
    <div class="row">
        <div class="col-md-3 col-lg-2 sidebar d-flex flex-column p-3 text-white bg-dark" style="min-height: 100vh;">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                ...
                <li><a href="index.php?url=categorias/listar" class="nav-link active"><i class="bi bi-tags"></i> Categorías</a></li>
        ...
        <div class="col-md-9 ms-sm-auto col-lg-10 main-content">
            <div class="d-flex justify-content-between pt-3 pb-2 mb-3 border-bottom">
                <h1 class="h2">Módulo de Categorías</h1>
                <a href="index.php?url=categorias/crearForm" class="btn btn-success"><i class="bi bi-plus-circle"></i> Nueva Categoría</a>
            </div>
            <div class="card shadow-sm"><div class="card-body"><table class="table table-hover table-bordered align-middle">
                <thead class="table-dark"><tr><th>ID</th><th>Nombre</th><th class="text-center">Acciones</th></tr></thead>
                <tbody>
                    <?php if(empty($categorias)): ?><tr><td colspan="3" class="text-center">No hay registros</td></tr><?php else: ?>
                    <?php foreach($categorias as $c): ?><tr><td><?= htmlspecialchars($c["id"]) ?></td><td><strong><?= htmlspecialchars($c["nombre"]) ?></strong></td>
                    <td class="text-center"><a href="index.php?url=categorias/editarForm&id=<?= $c["id"] ?>" class="btn btn-sm btn-primary"><i class="bi bi-pencil"></i></a> <a href="index.php?url=categorias/eliminarForm&id=<?= $c["id"] ?>" class="btn btn-sm btn-danger"><i class="bi bi-trash"></i></a></td></tr><?php endforeach; endif; ?>
                </tbody>
            </table></div></div>
        </div>
    </div>
